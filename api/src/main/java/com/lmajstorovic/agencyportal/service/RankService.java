...
public class RankService {
    ...
    public Rank getRankByName(String name) {
        ...
        if (rankOptional.isPresent()) {
            ...
            throw new IllegalStateException("User with rank " + name + " does not exist");
        }
    }

    public Rank getRankByDivision(UUID idDivision) {
        Optional<Rank> rankOptional = rankRepository.findRankByDivision(idDivision);
        if (rankOptional.isPresent()) {
            return rankOptional.get();
        } else {
            throw new IllegalStateException("Division with id " + idDivision + " does not exist");
        }
    }
}
