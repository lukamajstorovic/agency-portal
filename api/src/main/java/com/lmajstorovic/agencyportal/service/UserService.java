...
public class UserService implements UserDetailsService {
   ...
   /* @Autowired
    ...
    public void deleteUser(UUID userId) {
        ...
        if (!userExists) {
            throw new IllegalStateException("User with id " + userId + " does " +
                "not exist");
        }
    ...
    public User getUserById(UUID id) {
        ...
        if (user.isPresent()) {
            ...
            throw new IllegalStateException("User with id " + id + " does not " +
                "exist");
        }
    }
    ...
    public List<User> getUsersByRank(UUID idRank) {
        ...
        if (!userCollection.isEmpty()) {
            ...
            throw new IllegalStateException("Users with rank " + idRank + " do " +
                "not exist");
        }
    }
    ...
    public void updateUsername(
        ...
        if (user.isPresent()) {
            ...
            if (username.isBlank()) {
                ...
                throw new IllegalStateException("New username can not be the same" +
                    " as the old username");
            ...
            throw new IllegalStateException("User with id " + userId + " not " +
                "found");
        }
    }
    ...
    public void updateTag(
        ...
        if (user.isPresent()) {
            ...
            if (tag.contains("[]")) {
                ...
                throw new IllegalArgumentException("Tag must be between 2 and 4 " +
                    "characters long");
            } else if (Objects.equals(
                existingUser.getTag(),
                tag
            )) {
                throw new IllegalStateException("New tag can not be the same as " +
                    "the old tag");
    ...
    @Transactional
    public void updatePersonalSecretary(
        UUID userId,
        String secretaryUsername
    ) {
        Optional<User> user = userRepository.findById(userId);
        Optional<User> secretary =
            userRepository.findUserByUsername(secretaryUsername);
        User existingUser;
        User existingSecretary;
        if (user.isPresent()) {
            existingUser = user.get();
            if (secretaryUsername.isBlank()) {
                throw new IllegalArgumentException("Secretary username can not be" +
                    " empty");
            } else if (secretary.isPresent()) {
                existingSecretary = secretary.get();
                if (existingSecretary.getId() == existingUser.getIdPersonalSecretary()) {
                    throw new IllegalStateException("User is already marked as the" +
                        " personal secretary");
                } else {
    ...
    public void updateActiveStatus(
        ...
        if (user.isPresent()) {
            existingUser = user.get();
            if (active == null) {
                throw new IllegalArgumentException("Active can not be null");
            } else if (existingUser.getActive() == active && active) {
                if (active) {
                    throw new IllegalStateException("User is already active");
                } else {
                    throw new IllegalStateException("User is already not active");
                }
            } else {
